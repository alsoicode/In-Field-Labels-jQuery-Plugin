...
*Modified By:* "Brandon Taylor":http://btaylorweb.com/
...
This fork adds:
An object parameter to position the label: <code>{ position : {'left' : 0, 'top' : 0} }</code>
More HTML5 selectors instead of just <code>type["text"]</code> and <code>textarea</code>. See "src/jquery.infieldlabel.js":https://github.com/btaylordesign/In-Field-Labels-jQuery-Plugin/blob/master/src/jquery.infieldlabel.js for a list of supported input types.
...
Four options can be passed along with the method or set ahead of time for all inFieldLabel controls.
...
*<code>labelClass:</code> String to be used as class*
Class to be applied to the label when it is positioned over form field. Defaults to infield

*<code>labelPosition:</code> Object to position label within field. Defaults to {position : {'left' : 0, 'top' : 0}}*
Absolute position to be applied to the label as an inline style.
